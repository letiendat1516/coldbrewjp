...
router.post(
  ...
  [body('className').trim().notEmpty().withMessage('Class name is required'), validate],
...
router.post(
  '/join',
  authorize('STUDENT'),
  [body('joinCode').trim().notEmpty().withMessage('Join code is required'), validate],
  classCtrl.joinClass
);
...
router.delete('/:id/members/:studentId', authorize('TEACHER', 'ADMIN'), classCtrl.removeMember);
